...
**Total placeholders:** 28 across 23 unique filenames
...
| `date`     | `usage/date-time-calculation.md`         | time-calculation.png: The Date/Time calculation dialog open with Operation = "Difference between two dates", two date columns picked, Result unit = Days, and a New column name filled in.                                                                                                                                                |
...
| `notebook` | `usage/view-modes/notebook.md`           | editing-source.png (optional): A notebook code cell mid-edit, text cursor in the source, the modified-dot showing on the tab title, and the cell's existing output still visible below to illustrate that outputs are kept on save.                                                                                                       |
...
| `settings` | `reference/languages.md`                 | language.png: Settings dialog, Appearance section, the Language dropdown open showing the list of language names in their own script (Deutsch, Espanol, Francais, ...).                                                                                                                                                                   |
...
- **`raw`** (usage/view-modes/raw-text.md): text-view.png: Raw view of a Python file with syntect highlighting on. Show line numbers, the gutter, syntax-highlighted keywords/strings.
- **`sql`** (usage/sql.md): view.png: SQL panel docked at the bottom of the window. Editor on top with a multi-line SELECT query, result table below showing a few rows. Line numbers in the editor gutter, autocomplete chip row visible under the editor.
- **`chat`** (usage/chatbot.md): chat-panel.png: The chat assistant docked on the right of the table view. A user has asked "summarise the table I have open" and the assistant has replied with a short summary, with a collapsed "Tool call: profile" disclosure row visible above the answer.
- **`chat`** (usage/chatbot.md): chat-tab-chips.png: The chat panel header with two open tabs shown as chips ("#1 sales.csv" highlighted as active, "#2 returns.csv"), illustrating how the assistant addresses multiple open tables.
