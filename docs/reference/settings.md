...
| **Write options**             | (see below) | How files are written, in one expander per format. **Parquet**: compression, rows per row group, dictionary encoding, column statistics. **CSV / TSV**: delimiter, quoting, line endings, header row. **Excel**: include formatting, keep formulas, document properties, write as an Excel table. Parquet is written with `zstd` unless you change it; the other defaults reproduce Octa's previous behaviour. Also used by command-line conversions that do not pass `--compression` / `--row-group-size`. Batch convert can override them per run. TOML key: `write_options`. |
...
| **Cell line breaks**        | off       | Render `\n` inside cells as actual line breaks. Rows have variable height when on. Switched on automatically by **Edit > Auto-fit All Rows** and by double-clicking a row seam, since fitting a row means wrapping it.                                           |
...
| **Max tokens**                 | 16,384                   | Cap on the model's response length. **Unlimited** omits the field; Anthropic requires one, so there it becomes the ceiling the Models API reports for that model (128k on the Claude 5 generation), falling back to 16,384 if that cannot be read. TOML keys: `chat_max_tokens`, `chat_max_tokens_unlimited`.                                                                          |
...
| **Render Markdown in replies** | on                       | Draw headings, lists, tables, code blocks and links in the assistant's replies as formatting. Off shows the raw Markdown source. Your own messages, tool arguments and tool results are never reformatted. TOML key: `chat_render_markdown`.                                                                                                                                           |
...
| Setting                            | Default | Notes                                                                                                                                                                   |
|------------------------------------|---------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Check for updates at start**     | on      | One GitHub request per launch. Downloads and installs nothing; silent when up to date or offline. Greyed out in a Microsoft Store copy, which the Store updates itself. |
