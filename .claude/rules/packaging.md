...
- **`scripts/build-store-listing.py`** fills the Partner Center listing CSV. The export has a `default` column plus one per language **detected in the uploaded package** (hence the 32 `<Resource Language>` entries in `AppxManifest.xml`); blank cells inherit `default`, so one English set covers all 32 - which is why the script **blanks** any *text* cell `_default.toml` owns (a filled cell beats `default`). **Assets are the opposite**: Partner Center stores an image per language listing (an export shows a dashboard URL in every column), so a blank cell there means that listing has no picture - the script writes the screenshot/logo paths and `OverrideLogosForWin10` into all 34 columns. Content in `docs/assets/store/listings/<code>.toml` (31 files, native, informal; `en-us` inherits `_default.toml`; `sr` -> `sr-cyrl.toml`). It drops the Xbox/Holographic/SurfaceHub/mobile rows (453 -> 225) but **never trailer rows** (deleting one deletes the asset in Partner Center), and leaves `Field`/`ID`/`Type` untouched. **The listing never says Excel** - not as a keyword, not in the prose; the format is `XLSX` everywhere. `excel alternative` in `SearchTerm7` cost the 0.19.1 submission a review cycle ("keywords which are product titles that aren't published by you", flagged in all 32 languages at once), so the script now exits 1 on any Excel spelling in any cell, and on `power bi`/`google sheets`/`sql server` in a keyword.
...
**Store handling**: a Store (MSIX) copy gets **no update check at all**. `octa::platform::is_store_packaged()` (`src/platform.rs`, a cached `\WindowsApps\` path check, no Windows API) hides the Help entry (`src/ui/toolbar/help_menu.rs`), skips the startup check (`src/app/update_loop.rs`) and greys out `check_updates_on_start` in Settings with `release.store` as the disabled hint; `src/app/dialogs/update_dialog.rs` keeps its `Update now` gate as a last guard for a caller that should no longer exist. 0.17.0 tried the opposite - entry and check on, only the install button gated, on the theory that knowing a release exists is useful there - and 0.19.0 in the field proved it wrong: the packaged process fails the `api.github.com` request with a certificate/timestamp error, so the entry only ever produced a scary error about an update the Store was already installing. **Do not re-open it.** The release-notes window needs no branch either way, having never offered an install and shipping its notes inside the binary. Status bar splits `Checking` from `Updating` (a spinner labelled "Updating..." for a read-only version query reads as a silent install).
