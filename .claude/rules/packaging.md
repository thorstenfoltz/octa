...
**Startup check + release notes.** Two settings, both default **on**: `check_updates_on_start` (one request per launch) and `show_release_notes` (the window). `update_loop::ui` fires the check on the first frame it sees (needs an `egui::Context` to repaint, so not `OctaApp::new`) guarded by `startup_update_started`, and `drain_startup_update_check` acts on the result once (`startup_update_seen`). `UpdateState::Available{version, notes}` + notes on + `last_release_notes_version != version` -> `pending_release_notes` -> `src/app/dialogs/release_notes.rs` (notes rendered by the shared `view_modes::markdown::render_pulldown`; **Update now** hands off to `perform_update`; closing records `last_release_notes_version` so it is once per **release**, not per launch). Notes off -> a `status_message` toast, so the check is never a silent no-op. **`UpToDate` carries `{notes}` too** and raises the SAME window for the version the user is *running* when `last_release_notes_version != CARGO_PKG_VERSION` - otherwise the feature could never fire for the release that introduced it. That form drops the Update button and the store note (`current` flag) and titles itself `release.whats_new`; with notes off it stays silent. `Error` stays silent: a flaky network must not nag at every launch. The in-window "Do not show this again" checkbox writes `show_release_notes = false`.

**Store handling**: knowing a release exists is useful on Store even though the *install* is impossible there, so `octa::platform::is_store_packaged()` no longer suppresses the check or hides the Help entry - it only makes **both dialogs drop the Update button and show `release.store`**. Status bar splits `Checking` from `Updating` (a spinner labelled "Updating..." for a read-only version query reads as a silent install).
