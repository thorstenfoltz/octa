...
fn sections(shortcuts: &ui::shortcuts::Shortcuts) -> Vec<(&'static str, String)> {
    ...
    vec![
        ...
        ("Online Documentation", ONLINE_DOCS.to_string()),
        ...
        ("Ask (plain language filter)", ASK_FILTER.to_string()),
        ("Column Filter", COLUMN_FILTER.to_string()),
        ("Column Tools", COLUMN_TOOLS.to_string()),
        ("Parse in New Tab", PARSE_IN_NEW_TAB.to_string()),
        ...
        ("File Internals", FILE_INTERNALS.to_string()),
        ("Join Key Finder", JOIN_KEYS.to_string()),
        ("Join Diagnostics", JOIN_DIAG.to_string()),
        ("Schema Drift", SCHEMA_DRIFT.to_string()),
        ("Data Drift", DATA_DRIFT.to_string()),
        ("Relationship Map", REL_MAP.to_string()),
        ("Large Files", LARGE_FILES.to_string()),
        ("Harmonise Schemas", HARMONISE.to_string()),
        ("Report", REPORT.to_string()),
        ("Fuzzy Join", FUZZY_JOIN.to_string()),
        ("Compare with Database", DB_COMPARE.to_string()),
        ("Clean-up Suggestions", CLEANUP.to_string()),
        ("Date/Time Calculation", DATE_TIME_CALC.to_string()),
        ("Time Series", TIME_SERIES.to_string()),
        ("Pivot / Unpivot", PIVOT.to_string()),
        ("Correlation", CORRELATION.to_string()),
        ("Compare distributions", DIST_COMPARE.to_string()),
        ("Referential integrity", REFERENTIAL.to_string()),
        ...
        ("Jump to Flagged Cells", PROBLEM_NAV.to_string()),
        ("Filter to Marked", FILTER_TO_MARKED.to_string()),
        ("Rename Columns", RENAME_COLUMNS.to_string()),
        ("Bookmarks", BOOKMARKS.to_string()),
        ("Transform Column", TRANSFORMS.to_string()),
        ("Anonymise Columns", ANONYMIZE.to_string()),
        ("Detect Outliers", OUTLIERS.to_string()),
        ("Detect PII", PII.to_string()),
        ("Union Tables", UNION.to_string()),
        ("Join Tables", JOIN.to_string()),
        ("Partition by Column", PARTITION.to_string()),
        ("Batch Convert", BATCH_CONVERT.to_string()),
        ("Table Tools", TABLE_TOOLS.to_string()),
        ("Clean Headers on Load", CLEAN_HEADERS.to_string()),
        ("Sorting", SORTING.to_string()),
        ("View Modes", VIEW_MODES.to_string()),
        ("Record View", RECORD_VIEW.to_string()),
        ...
        ("Compressed Files", COMPRESSED_FILES.to_string()),
        ("Datasets (Folder of Parts)", DATASETS.to_string()),
        ("Cloud Storage", CLOUD_STORAGE.to_string()),
        ("Cloud Inventory", CLOUD_INVENTORY.to_string()),
        ("Database Connections", DATABASES.to_string()),
        ...
        ("Export to PDF", PDF_EXPORT.to_string()),
